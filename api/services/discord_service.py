...
class DiscordWebhookService:
    ...
    def __init__(self, goose_executor: Optional[TaskExecutor] = None):
        ...
        Args:
            goose_executor (Optional[TaskExecutor], optional): Goose実行ラッパーインスタンス。デフォルトはNone
        """
        self.logger = logging.getLogger("discord_webhook_service")
        self.goose_executor = goose_executor or TaskExecutor()
        self.task_service = TaskService(self.goose_executor)
    ...
    async def handle_webhook(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if webhook_type == "message":
                return await self._handle_message_webhook(data)
            elif webhook_type == "reaction":
                return await self._handle_reaction_webhook(data)
            else:
                return {
                    "success": False,
                    "message": f"未対応のWebhookタイプ: {webhook_type}",
                }

        except Exception as e:
            self.logger.error(f"Webhook処理エラー: {str(e)}")
            return {"success": False, "message": f"Webhook処理エラー: {str(e)}"}

    async def _handle_message_webhook(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """メッセージWebhookを処理
        ...
        result = await self.task_service.create_task(
            ...
            prompt=f"Discordメッセージに応答してください: {content}",
            context={
                "message": content,
                "author": author,
                "channel": channel,
        ...
        return {
            ...
            "message": ("メッセージを処理しました" if result["success"] else "メッセージの処理に失敗しました"),
        }

    async def _handle_reaction_webhook(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """リアクションWebhookを処理
        ...
        # 鉛筆リアクションの場合は要約タスクを作成
        if emoji == "✏️":
            result = await self.task_service.create_task(
                task_type="discord_summary",
                prompt="会話を要約してください",
                context={
            ...
            return {
                "success": result["success"],
                "task_id": result["task_id"],
                "message": ("要約タスクを作成しました" if result["success"] else "要約タスクの作成に失敗しました"),
            }
        else:
            return {
                "success": True,
                "message": f"リアクション {emoji} は処理されません",
            }
